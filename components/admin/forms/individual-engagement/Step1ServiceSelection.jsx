...
import { Form, Checkbox, Radio, Alert, Tooltip, Tag } from "antd";
import {
  InfoCircleOutlined,
  CheckCircleFilled,
  FileTextOutlined,
...
const SERVICE_OPTIONS = [
  ...
  },
  {
    ...
    description: "Income, interest, depreciation, and deductions for investment properties",
    icon: <HomeOutlined className="text-emerald-600 dark:text-emerald-400" />,
  },
  {
    value: "Capital Gains",
    label: "Capital Gains Tax (CGT)",
    description: "Calculations for property sales, shares, investments, or assets",
  ...
  },
  {
    ...
    description: "Overseas employment, residency status, and foreign tax credits",
  ...
  },
  {
    ...
    description: "Representation for ATO reviews, audits, penalties, or payment plans",
    icon: <AuditOutlined className="text-emerald-600 dark:text-emerald-400" />,
  },
  {
    value: "Sole Trader BAS",
    label: "Sole Trader BAS Lodgement",
    description: "Quarterly or annual Business Activity Statements for sole traders",
    icon: <FileTextOutlined className="text-emerald-600 dark:text-emerald-400" />,
  },
  {
  ...
  },
  {
    ...
    icon: <PlusCircleOutlined className="text-emerald-600 dark:text-emerald-400" />,
  },
];

export default function Step1ServiceSelection({ form, formData, onValuesChange }) {
    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="border-b border-slate-100 dark:border-zinc-800 pb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-semibold text-slate-400 dark:text-zinc-500">Service Selection</span>
        </div>
        <h2 className="text-2xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight">
          What services do you require?
        </h2>
        <p className="text-sm text-slate-600 dark:text-zinc-400 mt-1">
          Please select the taxation, advisory, or compliance services you need help with today.
        </p>
      </div>

      {/* SRV-001: Services Selection */}
      <Form.Item
        name="services"
        rules={[
          {
        ...
        <Checkbox.Group className="w-full">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
            {SERVICE_OPTIONS.map((item) => {
              const isChecked = selectedServices.includes(item.value);
              return (
                <label
                  key={item.value}
                  className={`relative p-4 rounded-2xl cursor-pointer block select-none ${
                    styles.serviceOptionCard
                  } ${
                    isChecked
                      ? `${styles.selectedCard} bg-brand-primary-soft/60 dark:bg-emerald-950/60 border-brand-primary dark:border-emerald-500 shadow-md`
      ...
      {/* SRV-002: Entity Related Work Question */}
      <div className="p-6 rounded-2xl bg-slate-50/80 dark:bg-zinc-900/60 border border-slate-200/80 dark:border-zinc-800 space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-slate-900 dark:text-zinc-100">
              Does the service relate to a company, trust, partnership or another entity?
            </h3>
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400">
            Let us know if you also operate a business entity that requires tax lodgement or compliance.
          </p>
        </div>
          ...
          <Radio.Group className="w-full">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
              {[
                ...
                const isSelected = entityService === opt.value;
                return (
                  <label
                    key={opt.value}
                    className={`p-4 rounded-xl cursor-pointer block select-none ${
        ...
        {entityService === "Yes" && (
          <Alert
            type="info"
            showIcon
            message="Linked Entity Engagement Notice"
