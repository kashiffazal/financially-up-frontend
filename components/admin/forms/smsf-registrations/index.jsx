...
import Step1FundInfo from "./Step1FundInfo";
import Step2TrusteeStructure from "./Step2TrusteeStructure";
import Step3MembersInfo from "./Step3MembersInfo";
import Step4ComplianceSign from "./Step4ComplianceSign";

const DRAFT_STORAGE_KEY = "FINANCIALLY_UP_SMSF_REGISTRATIONS_DRAFT";

const STEP_ITEMS = [
  { step: 1, title: "Fund Profile", fullTitle: "Name & Founder" },
  { step: 2, title: "Trustee Model", fullTitle: "Corporate Trustee & Office" },
  { step: 3, title: "Members", fullTitle: "Trustees & TFN Details" },
  { step: 4, title: "Compliance", fullTitle: "ESA & Declaration" },
];

export default function SMSFRegistrationsForm() {
    ...
    try {
      ...
      if (savedDraftStr) {
        ...
        if (savedDraft && savedDraft.data) {
          ...
          message.info(`Restored your saved SMSF Registration progress.`);
        }
      }
    } catch (e) {
  ...
  const handleNext = async () => {
    try {
      ...
      if (currentStep < 3) {
        ...
        try {
          console.log("Submitting SMSF Payload:", mergedPayload);
              ...
              <div className="pt-2 pb-1 space-y-5 animate-fadeIn">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight">
                    SMSF Establishment Initiated Successfully
                  </h3>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-zinc-400 max-w-sm mx-auto leading-relaxed">
                    Your SMSF setup application for <strong className="text-brand-primary">{mergedPayload.NameOfSMSF}</strong> has been logged with our superannuation team.
                  </p>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-200/80 dark:border-zinc-800 space-y-3">
                  ...
                  <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-zinc-800 pb-2.5">
                    ...
                    <span className="text-xs font-bold text-slate-700 dark:text-zinc-300">
                      {mergedPayload.trusteeStructureType}
                    </span>
                  </div>
                ...
                <div className="p-3.5 rounded-xl bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 space-y-2 text-xs">
                  ...
                  <ul className="space-y-1.5 text-slate-600 dark:text-zinc-300 leading-normal pl-5 list-disc">
                    <li>We will draft your customized SIS-compliant SMSF Trust Deed.</li>
                    <li>Fund ABN/TFN and ESA integration will be submitted to the ATO.</li>
                  </ul>
                </div>
              </div>
            ),
  ...
  return (
    <Card className="shadow-lg border border-slate-200/80 dark:border-zinc-800 rounded-3xl overflow-hidden dark:bg-zinc-950">
      ...
      >
        {currentStep === 0 && <Step1FundInfo />}
        {currentStep === 1 && <Step2TrusteeStructure />}
        {currentStep === 2 && <Step3MembersInfo />}
        {currentStep === 3 && <Step4ComplianceSign />}
      </Form>
      ...
      <div className="mt-10 pt-6 border-t border-slate-100 dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 p-2 sm:p-6 bg-slate-50/50 dark:bg-zinc-900/40 rounded-2xl">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          >
            {currentStep === 3 ? "Submit SMSF Registration" : "Next: Step 4"}
          </Button>
        </div>
      </div>
    </Card>
