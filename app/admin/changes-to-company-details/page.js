...
const EXPORT_COLUMNS = [
  { header: "ID", key: "id" },
  { header: "Company Name", key: "NameOfCompany" },
  { header: "ACN/ABN", key: "ACNorABN" },
  { header: "Contact Name", key: "_contactName" },
...
export default function ChangesToCompanyDetailsPage() {
  ...
  const handleStatusChange = (record, newStatus) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await HTTP("PUT", `/changes-to-company-details/${record.id}`, {
            status: newStatus,
          });
          ...
          <div className="flex flex-wrap items-center justify-end gap-4 w-full md:w-2/3">
            ...
            <div className="flex flex-col">
              ...
              <ExportButtons
                ...
                filenamePrefix={`changes-to-company-details-${activeTab === "All" ? "All" : activeTab.replace(/\s+/g, "-")}`}
              />
            </div>
          </div>
        </div>
