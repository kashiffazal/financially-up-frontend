...
export default function CompanyRegistrationPage() {
  ...
  const [filterBy, setFilterBy] = useState("Company Name"); // Selected filter column
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await HTTP("GET", "/company-registrations", {
  ...
  const handleApproveSubmit = async (values) => {
    try {
      await HTTP("PUT", `/company-registrations/${currentRecord.id}`, {
        status: "Approved",
  ...
  const handleStatusChange = (record, newStatus) => {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          <p>
            ...
            <strong>{record.companyName || record.proposed_name}</strong> from{" "}
      ...
      onOk: async () => {
        try {
          await HTTP("PUT", `/company-registrations/${record.id}`, {
            status: newStatus,
          });
  ...
  const handleDelete = async (record) => {
    Modal.confirm({
      title: `Delete ${record.companyName || record.proposed_name}?`,
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-6 gap-4">
        <div>
          ...
          >
            <BankOutlined className="text-slate-500" /> Company Registration
          </Title>
          <Text className="text-slate-500 dark:text-slate-400">
            Manage company registration applications.
          </Text>
        </div>
        <Breadcrumb
          items={[
      ...
      <div className="bg-white dark:bg-zinc-950 p-6 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-2 mb-4">
          <div className="w-full md:w-1/3">
            ...
            >
              {activeTab === "All" ? "Regular" : activeTab} Company Registration
            </Title>
          ...
          <div className="flex flex-wrap items-center justify-end gap-4 w-full md:w-2/3">
            ...
            <div className="flex flex-col">
              ...
              <ExportButtons
                ...
                filenamePrefix={`company-registration-${activeTab === "All" ? "All" : activeTab.replace(/\s+/g, "-")}`}
              />
            </div>
          </div>
        </div>
