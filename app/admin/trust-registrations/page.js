...
const EXPORT_COLUMNS = [
  { header: "ID", key: "id" },
  { header: "Trust Name", key: "TrustName" },
  { header: "Type of Trust", key: "TypeOfTrust" },
  { header: "Trustee Name", key: "_trusteeName" },
...
export default function TrustRegistrationsPage() {
  ...
  const [filterBy, setFilterBy] = useState("Trust Name"); // Selected filter column
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await HTTP("GET", "/trust-registrations", {
  ...
  const handleApproveSubmit = async (values) => {
    try {
      await HTTP("PUT", `/trust-registrations/${currentRecord.id}`, {
        status: "Approved",
        approvalNotes: values.notes || null,
      });
      message.success(
        `${currentRecord.FirstName || currentRecord.TrustName || "Record"} approved successfully`,
      );
  ...
  const handleDelete = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await HTTP("DELETE", `/trust-registrations/${record.id}`);
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-6 gap-4">
        <div>
          ...
          >
            <BlockOutlined className="text-slate-500" /> Trust Registrations
          </Title>
          <Text className="text-slate-500 dark:text-slate-400">
            Manage trust registration applications.
          </Text>
        </div>
        <Breadcrumb
          items={[
      ...
      <div className="bg-white dark:bg-zinc-950 p-6 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-2 mb-4">
          <div className="w-full md:w-1/3">
            ...
            >
              {activeTab === "All" ? "Regular" : activeTab} Trust Registrations
            </Title>
          ...
          <div className="flex flex-wrap items-center justify-end gap-4 w-full md:w-2/3">
            <div className="flex flex-col">
              ...
              <Select
                ...
                options={[
                  ...
                  { value: "Type of Trust", label: "Type of Trust" },
                  { value: "Trustee Name", label: "Trustee Name" },
            ...
            <div className="flex flex-col">
              ...
              <ExportButtons
                ...
                filenamePrefix={`trust-registrations-${activeTab === "All" ? "All" : activeTab.replace(/\s+/g, "-")}`}
              />
            </div>
          </div>
        </div>
