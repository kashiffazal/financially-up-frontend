...
export default function BusinessNameRegistrationsPage() {
  ...
  const [filterBy, setFilterBy] = useState("Proposed Business Name"); // Selected filter column
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await HTTP("GET", "/business-name-registrations", {
  ...
  const handleApproveSubmit = async (values) => {
    try {
      await HTTP("PUT", `/business-name-registrations/${currentRecord.id}`, {
        status: "Approved",
        approvalNotes: values.notes || null,
      });
      antdMsg.success(
        `${currentRecord.FirstName || currentRecord.businessProposeName || "Record"} approved successfully`,
      );
  ...
  const handleStatusChange = (record, newStatus) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await HTTP("PUT", `/business-name-registrations/${record.id}`, { status: newStatus });
  ...
  const handleDelete = async (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await HTTP("DELETE", `/business-name-registrations/${record.id}`);
          ...
          <div className="flex flex-wrap items-center justify-end gap-4 w-full md:w-2/3">
            ...
            <div className="flex flex-col">
              ...
              <ExportButtons
                ...
                filenamePrefix={`business-name-registrations-${activeTab === "All" ? "All" : activeTab.replace(/\s+/g, "-")}`}
              />
            </div>
          </div>
        </div>
