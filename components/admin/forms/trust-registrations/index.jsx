...
import Step1TrustProfile from "./Step1TrustProfile";
import Step2TrusteeAppointer from "./Step2TrusteeAppointer";
import Step3BeneficiariesUnits from "./Step3BeneficiariesUnits";
import Step4DeclarationExecution from "./Step4DeclarationExecution";

const DRAFT_STORAGE_KEY = "FINANCIALLY_UP_TRUST_REGISTRATIONS_DRAFT";

const STEP_ITEMS = [
  { step: 1, title: "Trust Profile", fullTitle: "Structure & State" },
  { step: 2, title: "Trustee & Settlor", fullTitle: "Corporate Trustee & Office" },
  { step: 3, title: "Beneficiaries", fullTitle: "Units & Share Allocation" },
  { step: 4, title: "Execution", fullTitle: "Deed & Consent" },
];

export default function TrustRegistrationsForm() {
    ...
    try {
      ...
      if (savedDraftStr) {
        ...
        if (savedDraft && savedDraft.data) {
          ...
          message.info(`Restored your saved Trust Registration progress.`);
        }
      }
    } catch (e) {
  ...
  const handleNext = async () => {
    try {
      ...
      if (currentStep < 3) {
        ...
        try {
          console.log("Submitting Trust Registration Payload:", mergedPayload);
              ...
              <div className="pt-2 pb-1 space-y-5 animate-fadeIn">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight">
                    Trust Establishment Initiated Successfully
                  </h3>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-zinc-400 max-w-sm mx-auto leading-relaxed">
                    Your trust establishment setup for <strong className="text-brand-primary">{mergedPayload.TrustName}</strong> ({mergedPayload.TypeOfTrust}) has been received.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 space-y-2 text-xs">
                  ...
                  <ul className="space-y-1.5 text-slate-600 dark:text-zinc-300 leading-normal pl-5 list-disc">
                    <li>We will prepare your tailored Trust Deed & Settlor execution package.</li>
                    <li>Trust ABN/TFN applications will be lodged with the ATO.</li>
                  </ul>
                </div>
              </div>
            ),
  ...
  return (
    <Card className="shadow-lg border border-slate-200/80 dark:border-zinc-800 rounded-3xl overflow-hidden dark:bg-zinc-950">
      ...
      >
        {currentStep === 0 && <Step1TrustProfile />}
        {currentStep === 1 && <Step2TrusteeAppointer />}
        {currentStep === 2 && <Step3BeneficiariesUnits />}
        {currentStep === 3 && <Step4DeclarationExecution />}
      </Form>
      ...
      <div className="mt-10 pt-6 border-t border-slate-100 dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 p-2 sm:p-6 bg-slate-50/50 dark:bg-zinc-900/40 rounded-2xl">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          >
            {currentStep === 3 ? "Submit Trust Registration" : "Next: Step 4"}
          </Button>
        </div>
      </div>
    </Card>
