...
import { Table, Tag, Button, Alert, Modal, Card, Descriptions, Divider } from "antd";
import {
  FilePdfOutlined,
  CheckCircleFilled,
  EyeOutlined,
...
export const getScheduleItemsForServices = (services = []) => {
  ...
  return services.map((srv, idx) => {
    ...
    let includedWork = "Data verification, ATO portal pre-fill check, tax calculation, electronic lodgement, and notice of assessment review.";
    let excludedWork = "ATO audit defense, legal advisory, bookkeeping/data entry, or financial planning advice.";
    let infoDeadline = "15 October";
    let lodgmentDeadline = "31 October";
    let specialConditions = "Client must provide all income statements, interest records, and deduction receipts prior to preparation.";
    let period = fyPeriod;

    if (srv.includes("Rental Property")) {
      fee = "+ $120.00 / property (ex. GST)";
      deliverable = "Rental Property Schedule & Capital Works Advice";
      specialConditions = "Client must supply 12-month property manager annual statement and loan interest statements.";
    } else if (srv.includes("Capital Gains")) {
      fee = "+ $150.00 / CGT event (ex. GST)";
      deliverable = "Capital Gains Tax Schedule & Discount Calculation";
      specialConditions = "Client must supply purchase contract, settlement statements, and cost base records.";
      ...
      specialConditions = "Client must supply complete exchange transaction CSV logs or API read access.";
      ...
      specialConditions = "Client must maintain reconciled bank records or software file (Xero/MYOB/QuickBooks).";
      ...
      specialConditions = "Subject to Registrar identification checks and business entity eligibility.";
      ...
      infoDeadline = "Immediate upon engagement";
      lodgmentDeadline = "Within 3 business days of receipt";
      specialConditions = "Client must confirm projected turnover exceeds $75,000 threshold.";
      ...
      specialConditions = "May involve ATO failure to lodge penalties which remain client responsibility.";
    } else if (srv.includes("Tax Return Amendment")) {
      fee = "$150.00 / amendment (ex. GST)";
      deliverable = "Notice of Assessment Amendment Request";
      specialConditions = "Requires copy of original notice of assessment and justification documents.";
      ...
      specialConditions = "Scope to be finalized in writing after initial document review.";
    }
    ...
    return {
      ...
      excludedWork,
      infoDeadline,
      expectedCompletion: "14 business days from receipt of complete information",
      lodgmentDeadline,
      fee,
      gstTreatment: "10% GST applies to all professional fees in Australia",
      urgentWorkLimit: "Urgent requests (< 5 business days) incur a 30% rush surcharge",
      specialConditions,
...
export default function Step8EngagementSchedule({
  ...
  const fullName = form?.getFieldValue("fullName") || formData?.fullName || "Individual Client";
  const selectedServices = form?.getFieldValue("services") || formData?.services || [];
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="border-b border-slate-100 dark:border-zinc-800 pb-4">
        ...
        <p className="text-sm text-slate-600 dark:text-zinc-400 mt-1">
          ...
          <strong className="text-slate-900 dark:text-zinc-200">{fullName}</strong>.
        </p>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-gradient-to-r from-emerald-50/80 via-slate-50 to-emerald-50/40 dark:from-emerald-950/40 dark:via-zinc-900 dark:to-emerald-950/20 border border-emerald-200/80 dark:border-emerald-800/50 shadow-md">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-1.5 max-w-xl">
            ...
            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              Contains all 15 statutory parameters including Service Scope, Exclusions, Information Deadlines, Lodgement Deadlines, Fee Breakdown, GST Treatment, and Responsible Tax Agent details.
            </p>
            <div className="flex flex-wrap items-center gap-3 pt-1 text-xs font-mono text-slate-500 dark:text-zinc-400">
              <span><strong>Client:</strong> {fullName}</span>
              <span>•</span>
              <span><strong>Term:</strong> Ongoing</span>
              <span>•</span>
              <span><strong>Date:</strong> {new Date().toLocaleDateString("en-AU")}</span>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-bold text-slate-900 dark:text-zinc-100 flex items-center gap-2">
            <LockOutlined className="text-brand-primary" /> Engagement Summary Overview
          </h3>
        ...
        {scheduleData.map((item, index) => (
          <Card
            key={item.key}
            className="rounded-2xl border border-slate-200/80 dark:border-zinc-800 shadow-xs hover:shadow-md transition-all overflow-hidden dark:bg-zinc-950"
            title={
              <div className="flex flex-wrap items-center justify-between gap-2 py-1">
                ...
                <Tag color="green" className="font-black text-xs px-3 py-0.5 rounded-full border-none">
                  Fee: {item.fee}
                </Tag>
              </div>
            }
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-semibold text-slate-800 dark:text-zinc-200">{item.period}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-semibold text-slate-800 dark:text-zinc-200">{item.deliverable}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-semibold text-slate-800 dark:text-zinc-200">{item.expectedCompletion}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-extrabold text-slate-800 dark:text-zinc-200">{item.infoDeadline}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-extrabold text-slate-800 dark:text-zinc-200">{item.lodgmentDeadline}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 space-y-1">
                ...
                <span className="font-semibold text-slate-800 dark:text-zinc-200">{item.gstTreatment}</span>
              </div>
              ...
              <div className="p-3 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 md:col-span-2 space-y-1">
                ...
                <p className="text-slate-700 dark:text-zinc-300 m-0 leading-relaxed">{item.includedWork}</p>
              </div>
              ...
              <div className="p-3 rounded-xl bg-rose-50/50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/40 space-y-1">
                ...
                <p className="text-slate-700 dark:text-zinc-300 m-0 leading-relaxed">{item.excludedWork}</p>
              </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-100 dark:border-zinc-800/80 md:col-span-3 space-y-1">
                ...
                <p className="text-slate-700 dark:text-zinc-300 m-0 italic">{item.specialConditions}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      <div className="p-5 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 space-y-3 text-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div>
            <span className="text-slate-400 block font-semibold text-[10px]">RESPONSIBLE ACCOUNTANT</span>
            <span className="font-bold text-slate-800 dark:text-zinc-200">Financially Up — Registered Tax Agent Team</span>
          </div>
          <div>
            <span className="text-slate-400 block font-semibold text-[10px]">ENGAGEMENT TERM</span>
            <span className="font-bold text-slate-800 dark:text-zinc-200">Ongoing until terminated in writing (14 days notice)</span>
          </div>
          <div>
            <span className="text-slate-400 block font-semibold text-[10px]">URGENT WORK SURCHARGE</span>
            <span className="font-bold text-slate-800 dark:text-zinc-200">30% surcharge applies for requests under 5 business days</span>
          </div>
          <div>
            <span className="text-slate-400 block font-semibold text-[10px]">SCHEDULE ACCEPTANCE DATE</span>
            <span className="font-bold text-slate-800 dark:text-zinc-200">{new Date().toLocaleDateString("en-AU")}</span>
          </div>
        </div>
      </div>
      ...
      <Modal
        ...
        style={{ top: 20 }}
        footer={[
          <div key="footer-btns" className="flex items-center justify-between w-full px-2">
            <Button
        ...
        <div className="p-4 sm:p-8 space-y-6 text-slate-900 dark:text-zinc-100 font-sans print:p-0">
          ...
          <div className="border-b-2 border-brand-primary pb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 m-0">
                Registered Tax Agent Services | ABN 12 345 678 901 | TASA 2009 Compliant
              </p>
            </div>
          ...
          <div className="bg-slate-50 dark:bg-zinc-900 p-4 rounded-xl border border-slate-200 dark:border-zinc-800 text-center">
            ...
            <p className="text-xs text-slate-500 m-0 mt-1">
              Issued for <strong className="text-slate-800 dark:text-zinc-200">{fullName}</strong> under Tax Agent Professional Regulations
            </p>
          </div>
          ...
          <div className="space-y-6">
            {scheduleData.map((item, idx) => (
              <div key={item.key} className="border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                ...
                <div className="p-4 space-y-3 text-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">1. Service Name</strong>
                      <span>{item.service}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">2. Year / Period</strong>
                      <span>{item.period}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">3. Deliverable</strong>
                      <span>{item.deliverable}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">6. Information Deadline</strong>
                      <span className="font-bold text-amber-700 dark:text-amber-400">{item.infoDeadline}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">7. Expected Completion</strong>
                      <span>{item.expectedCompletion}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">8. Lodgment / Payment Deadline</strong>
                      <span className="font-bold text-emerald-700 dark:text-emerald-400">{item.lodgmentDeadline}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">9. Professional Fee</strong>
                      <span className="font-bold">{item.fee}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">10. GST Treatment</strong>
                      <span>{item.gstTreatment}</span>
                    </div>
                    <div>
                      <strong className="text-slate-500 block uppercase text-[10px]">11. Urgent-Work Surcharge Limit</strong>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <p className="text-slate-600 dark:text-zinc-300 m-0 leading-relaxed">{item.includedWork}</p>
                    </div>
                    <div>
                      <strong className="text-rose-700 dark:text-rose-400 block uppercase text-[10px]">
                        5. Excluded Work Scope
                      </strong>
                      <p className="text-slate-600 dark:text-zinc-300 m-0 leading-relaxed">{item.excludedWork}</p>
                    </div>
                  </div>

                  <div className="pt-2">
                    <strong className="text-slate-500 block uppercase text-[10px]">12. Special Conditions</strong>
                    <p className="text-slate-600 dark:text-zinc-300 m-0 italic">{item.specialConditions}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="bg-slate-50 dark:bg-zinc-900 p-4 rounded-xl border border-slate-200 dark:border-zinc-800 text-xs space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <strong className="text-slate-500 block text-[10px]">13. RESPONSIBLE ACCOUNTANT</strong>
                <span>Financially Up — Registered Tax Agent Team</span>
              </div>
              <div>
                <strong className="text-slate-500 block text-[10px]">14. ENGAGEMENT TERM</strong>
                <span>Ongoing until terminated in writing (14 days notice)</span>
              </div>
              <div>
                <strong className="text-slate-500 block text-[10px]">15. ACCEPTANCE DATE</strong>
