/**
 * Apply TFN & ABNs API Service
 * ====================================
 * Frontend service layer for Apply TFN & ABN CRUD operations.
 * Wraps all API calls to the backend so components don't need to
 * know about URLs, headers, or error handling.
 *
 * Usage in components:
 *   import { getRecords, updateRecord } from '@/services/applyTfnAbns.service';
 *   const data = await getRecords({ page: 1, status: "Approved" });
 */

import { apiFetch } from "./apiConfig";

const ENDPOINT = "/apply-tfn-abns";

/**
 * Fetch all apply TFN & ABN records with pagination, status filter, and search.
...
/**
 * Fetch a single apply TFN & ABN record by ID.
...
/**
 * Create a new apply TFN & ABN record.
...
/**
 * Update an existing apply TFN & ABN record.
...
/**
 * Delete an apply TFN & ABN record.
 * @param {number} id - Record ID
