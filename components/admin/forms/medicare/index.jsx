...
import Step2VisaExemption from "./Step2VisaExemption";
import Step3EvidenceUploads from "./Step3EvidenceUploads";
import Step4ConsentDeclaration from "./Step4ConsentDeclaration";

const DRAFT_STORAGE_KEY = "FINANCIALLY_UP_MEDICARE_EXEMPTION_DRAFT";

const STEP_ITEMS = [
  { step: 1, title: "Applicant", fullTitle: "Personal & Passport" },
  { step: 2, title: "Visa & Period", fullTitle: "Subclass & Claim Dates" },
  { step: 3, title: "Evidence Upload", fullTitle: "Passport & Visa" },
  { step: 4, title: "Declaration", fullTitle: "Tax Agent Consent" },
];

export default function MedicareExemptionForm() {
    ...
    try {
      ...
      if (savedDraftStr) {
        ...
        if (savedDraft && savedDraft.data) {
          ...
          message.info(`Restored your saved Medicare Exemption application.`);
        }
      }
    } catch (e) {
  ...
  const handleNext = async () => {
    try {
      ...
      if (currentStep < 3) {
        ...
        try {
          console.log("Submitting Medicare Exemption Payload:", mergedPayload);
              ...
              <div className="pt-2 pb-1 space-y-5 animate-fadeIn">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-xl font-extrabold text-slate-900 dark:text-zinc-50 tracking-tight">
                    Medicare Exemption Application Submitted
                  </h3>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-zinc-400 max-w-sm mx-auto leading-relaxed">
                    Your Medicare Levy Exemption (MS015) application for <strong className="text-brand-primary">{mergedPayload.firstName} {mergedPayload.lastName}</strong> has been logged.
                  </p>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-200/80 dark:border-zinc-800 space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-zinc-800 pb-2.5">
                    ...
                    <span className="text-sm font-mono font-extrabold text-brand-primary dark:text-emerald-400">
                      {mergedPayload.passportNumber} ({mergedPayload.countryOfPassport})
                    </span>
                  </div>
                  <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-zinc-800 pb-2.5">
                    <span className="text-xs font-semibold text-slate-500 dark:text-zinc-400">
                  ...
                  <div className="flex items-center justify-between pt-0.5">
                    ...
                    <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300 px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-900">
                      Pending Services Australia Lodgement
                    </span>
                  </div>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 space-y-2 text-xs">
                  ...
                  <ul className="space-y-1.5 text-slate-600 dark:text-zinc-300 leading-normal pl-5 list-disc">
                    <li>Our tax agents will complete your MS015 lodgement with Services Australia.</li>
                    <li>You will receive your MES certificate to attach to your Tax Return.</li>
                  </ul>
                </div>
              </div>
            ),
  ...
  return (
    <Card className="shadow-lg border border-slate-200/80 dark:border-zinc-800 rounded-3xl overflow-hidden dark:bg-zinc-950">
      ...
      >
        {currentStep === 0 && <Step1ApplicantInfo />}
        {currentStep === 1 && <Step2VisaExemption />}
        {currentStep === 2 && <Step3EvidenceUploads />}
        {currentStep === 3 && <Step4ConsentDeclaration />}
      </Form>
      ...
      <div className="mt-10 pt-6 border-t border-slate-100 dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 p-2 sm:p-6 bg-slate-50/50 dark:bg-zinc-900/40 rounded-2xl">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          >
            {currentStep === 3 ? "Submit Medicare Exemption" : "Next: Step 4"}
          </Button>
        </div>
      </div>
    </Card>
